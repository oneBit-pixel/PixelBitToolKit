...
import com.onePixel.spinner.databinding.PowerspinnerLayoutBodyBinding
...
//        this.setOnClickListener { showOrDismiss() }
    ...
    public constructor(context: Context, attributeSet: AttributeSet) : super(
        context,
        attributeSet
    ) {
    ...
    private fun setTypeArray(a: TypedArray) {
        a.apply {
            ...
            if (hasValue(R.styleable.PowerSpinnerView_spinner_popup_bottom_padding)) {
                padding.bottom =
                    getDimensionPixelSize(
                        R.styleable.PowerSpinnerView_spinner_popup_bottom_padding,
                        0
                    )
            }
            ...
            if (hasValue(R.styleable.PowerSpinnerView_spinner_popup_start_padding)) {
                padding.start =
                    getDimensionPixelSize(
                        R.styleable.PowerSpinnerView_spinner_popup_start_padding,
                        0
                    )
            }
            ...
            if (hasValue(R.styleable.PowerSpinnerView_spinner_popup_padding)) {
                val value =
                    getDimensionPixelSize(R.styleable.PowerSpinnerView_spinner_popup_padding, 0)
                padding.apply {
                    top = value
                    end = value
                    bottom = value
            ...
            if (hasValue(R.styleable.PowerSpinnerView_spinner_selected_item_background)) {
                spinnerSelectedItemBackground =
                    getDrawable(R.styleable.PowerSpinnerView_spinner_selected_item_background)
            }
    ...
    private fun applyWindowAnimation() {
        if (this.spinnerPopupAnimationStyle == NO_INT_VALUE) {
            when (this.spinnerPopupAnimation) {
                ...
                SpinnerAnimation.FADE -> this.spinnerWindow.animationStyle =
                    R.style.PowerSpinner_Fade

                SpinnerAnimation.BOUNCE -> this.spinnerWindow.animationStyle =
                    R.style.PowerSpinner_Elastic
    ...
    private var totalList: MutableList<Any> = mutableListOf()
    ...
    public fun <T : Any> setItems(itemList: List<T>) {
        println("setItems...")
        totalList.clear()
        totalList.addAll(itemList)
        mList.addAll(itemList)
        (mList as MutableList<String>).remove(preferenceName)
        (adapter as PowerSpinnerInterface<T>).setItems(mList as List<T>)
    }
    ...
    public fun notifyItemSelected(index: Int, changedText: CharSequence) {
        println("notifyItemSelected")
        this.selectedIndex = index
        mList.clear()
        mList.addAll(totalList)
        (mList as MutableList<String>).remove(changedText)
        ...
        println("text==>${(mList as List<String>)[2]}")
        println("size==>${mList.size}")
        println("total==>${totalList.size}")
        (adapter as PowerSpinnerInterface<String>).setItems(mList as List<String>)
        this.preferenceName.whatIfNotNullOrEmpty {
            PowerSpinnerPersistence.getInstance(context)
                .persistSelectedIndex(it, this.selectedIndex)
        }
    }
