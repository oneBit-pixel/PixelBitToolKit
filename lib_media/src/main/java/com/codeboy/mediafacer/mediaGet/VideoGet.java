...
public class VideoGet {
    ...
    public ArrayList<VideoContent> getAllVideoContent(Uri contentLocation) {
        ...
        cursor = videoContext.getContentResolver().query(contentLocation, Projections, null, null, "LOWER (" + MediaStore.Video.Media.DATE_MODIFIED + ") DESC");//DESC ASC
        try {
            cursor.moveToFirst();
            do {
                ...
                videoContent.setDateTaken(cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Video.Media.DATE_TAKEN)));

                videoContent.setDateAdd(cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Video.Media.DATE_ADDED)));

                videoContent.setDateModify(cursor.getLong(cursor.getColumnIndexOrThrow(MediaStore.Video.Media.DATE_MODIFIED)) * 1000L);
