 ...
 */
public abstract class XFloatView<VB extends ViewBinding> implements OnTouchListener {
...
//     * @param layoutId 布局的资源ID（最好是LinearLayout)
     * @return
     */
    private View initFloatRootView() {
        //获取浮动窗口视图所在布局
        mBinding = bindView(LayoutInflater.from(mContext));
    ...
    public void initFloatViewPosition() {
        ...
        int y = (Utils.getScreenHeight(getContext()) - mBinding.getRoot().getMeasuredHeight()) / 2 - mStatusBarHeight;
    ...
    public void updateViewPosition(int PosX, int PosY) {
        mWmParams.x = PosX;
        mWmParams.y = PosY;
        mWindowManager.updateViewLayout(mBinding.getRoot(), mWmParams);
    }
     ...
     */
    public void show() {
        if (mBinding != null && mWmParams != null && !mIsShow) {
            mWindowManager.addView(mBinding.getRoot(), mWmParams);
            mIsShow = true;
        }
    }
     ...
     * @return
     */
    public XFloatView<VB> setOnFloatViewClickListener(OnClickListener onClickListener) {
     ...
     * @return
     */
    public XFloatView<VB> setOnFloatViewMoveListener(OnFloatViewMoveListener onFloatViewMoveListener) {
    ...
    public <T extends View> T findViewById(int resId) {
        return mBinding != null ? (T) mBinding.getRoot().findViewById(resId) : null;
    }
